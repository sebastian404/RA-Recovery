...
int erase_image(char* partition_name) {
    ...
    if (partition == NULL) die("can't find %s partition", partition_name);

    out = mtd_write_partition(partition);
    if (out == NULL) die("could not estabilish write context for %s", partition_name);

    // do the actual erase, -1 = full partition erase
    erased = mtd_erase_blocks(out, -1);

    // erased = bytes erased, if zero, something borked
    if (!erased) die("error erasing %s", partition_name);
